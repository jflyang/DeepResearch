...
职责：创建任务 → 扩展 query → 并发搜索 → 去重 → 评分 → 保存。
...
from models.enums import DownloadStatus, SearchSource, SourceLevel, TaskMode, TaskStatus
...
class ResearchService:
    ...
    async def run_initial_research(self, task: ResearchTask) -> ResearchResultSummary:
        ...
        raw_results, search_errors = await self._collect_search_results(expanded)
        ...
        # 5. 转换为 SourceItem
        source_items = self._to_source_items(task.id, scored)
    ...
    def _to_source_items(self, task_id: str, scored_results: list) -> list[SourceItem]:
        """将评分结果转换为 SourceItem 列表。"""
